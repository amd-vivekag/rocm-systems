...
///
/// Routes directly to the L1 Scalar Cache (K$) for loads, and bypasses
/// K$ (write-through to L2) for stores.
class ScalarMemPipeline : public MemoryPipeline {
public:
  /// @param l1 L1 Scalar Cache (K$), not owned.
  /// @param l2 L2 cache (for store bypass), not owned.
  ScalarMemPipeline(L1ScalarCache *l1, L2Cache *l2)
      : MemoryPipeline(WaitCounterType::LGKMCNT), l1_(l1), l2_(l2) {}
