...
class CodeGenerator:
    ...
    def _gen_execute_body(self, inst: Instruction, sem: InstructionSemantics, enc_name: str = '') -> str:
        ...
        if cls == 'dcache_wb':
            # K$ is read-only; no dirty lines to write back. No-op.
            return '  (void)wf;'
        ...
        return f'  (void)wf; // unhandled semantic class: {cls}'
    ...
    def _gen_flat_atomic(self, dst: list[str], src: list[str],
        ...
        if sem.operation is None or sem.operation not in self._ATOMIC_OP_ENUM:
            return f'  (void)wf; // TODO: unhandled flat_atomic variant ({sem.name})'
    ...
    def _gen_buffer_atomic(self, dst: list[str], src: list[str],
        ...
        if sem.operation is None or sem.operation not in self._ATOMIC_OP_ENUM:
            return f'  (void)wf; // TODO: unhandled buffer_atomic variant ({sem.name})'
    ...
    def _gen_ds_atomic(self, dst: list[str], src: list[str],
        ...
        if sem.operation is None or sem.operation not in self._ATOMIC_OP_ENUM:
            return f'  (void)wf; // TODO: unhandled ds_atomic variant ({sem.name})'
