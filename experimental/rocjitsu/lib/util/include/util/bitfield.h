...
public:
  ...
  T get() const {
    ...
    T bits{0};
    const T *bit_array(reinterpret_cast<const T *>(data));

    if (split) {
      int first_bits(elem_bits - bit_offset);
      int last_bits(n - first_bits);

      bits |= ((bit_array[first_elem] >> bit_offset) & mask<T>(first_bits));
      bits |= ((bit_array[last_elem] & mask<T>(last_bits)) << first_bits);
    } else {
      bits |= ((bit_array[first_elem] >> bit_offset) & mask<T>(n));
    }
